                                ...
                                <div class="row m-0">
                                            ...
                                            <div class="guest-profile">
                                                ...
                                                <div class="d-flex">
                                                    <div class="mt-4 check-status">
                                                        ...
                                                        <span class="font-w500 fs-16">{{ App\Models\User::lastCheckInDate($user->guests->id)  }}</span>
                                                    </div>
                                                    <div class="mt-4">
                                                        <span class="d-block mb-2">Check Out</span>	
                                                        <span class="font-w500 fs-16">{{ App\Models\User::lastCheckOutDate($user->guests->id) }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            @if ($hasCurrentBooking)
                                                <div class="d-flex flex-wrap">
                                                    <div class="mt-4 check-status">
                                                        ...
                                                        <h4 class="font-w500 fs-24">{{ $lastBooking->room->room_types->name }}</h4>
                                                    </div>
                                                    <div class="mt-4 ms-3">
                                                        <span class="d-block mb-2 text-black">Price</span>	
                                                        <span class="font-w500 fs-24 text-black">K{{ $lastBooking->room->room_types->price }}<small class="fs-14 ms-2 text-secondary">/{{ $lastBooking->room->room_types->per }}</small></span>
                                                    </div>
                                                </div>
                                                <div class="facilities">
                                                    <div class="my-2">
                                                        <p>{{ $lastBooking->special_requests ?? '' }}</p>
                                                    </div>
                                                    <div class="mb-3 ">
                                    ...
                                    <div class="col-xl-6 p-0">
                                            ...
                                            @else
                                                ...
                                                <div class="py-4">
                                                    <h3 class="mt-4">Last Reservation Sent</h3>
                                                    <div>
                                                        <span class=" text-secondary fs-14 d-block">Sent on {{ $lastReservation->created_at->toFormattedDateString() }}</span>
                                                        <span class=" fs-16 font-w500 text-nowrap">Reservation# {{ $lastReservation->id }}</span>
                                                    </div>
                                                    <br>
                                                    <div class="booked">
                                                        <p class="text-sm text-primary">{{ $lastReservation->is_confirmed == 1 ? 'Responded': 'Pending for response' }}</p>
                                                    </div>
                                                    <div class="img-content">
                                                        <div class="d-flex">
                                                            <div class="w-1/2 check-status">
                                                                <small>Check In Date</small>
                                                                <h5 class="font-w600 text-success">{{ $lastReservation->checkin_date }}</h5>	
                                                            </div>
                                                            <div class="w-1/2">
                                                                <small>Check Out Date</small>
                                                                <h5 class="font-w600 text-success">{{ $lastReservation->checkout_date }}</h5>
                                                            </div>	
                                                        </div>
                                                    ...
                                                    @if ($lastReservation->is_confirmed == 1)
                                                        ...
                                                        <button wire:loading.remove wire:click="bulkResponse({{ $lastReservation->id }})" data-bs-toggle="modal" data-bs-target="#respondToReservation" class="btn btn-secondary btn-xs">
                                                ...
                                                <div class="item">
                                                    <div class="rooms">
                                                        ...
                                                        <div class="img-content">
                                                            <h4 class="fs-24 font-w500 text-white">No Bookings</h4>
                                                            <p class="text-white">Customer has no booking yet.</p>	
                                                        </div>
                                                    </div>
                                                </div>
                                                @endif
                        ...
                        <div class="col-xl-12">
                            <div class="card">
                                <div class="card-header border-0">
                                    <h4 class="fs-20">Booking History</h4>
                                        ...
                                        <div class="row align-items-center">
                                            <div class="col-xl-4 col-sm-7">
                                                <div class="purchase-history d-flex align-items-center">
                                                    @if($book->room->room_types->cover != null)
                                                    <img class="rounded" src="{{ '../public/'.Storage::url($book->room->room_types->cover) }}" alt="">
                                                    @else
                                                    <img class="rounded" src="data:image/png;base64," alt="">
                                                    @endif 
                                                    <div class="ms-3">
                                                        <h4 class="guest-text font-w500">{{ $book->room->room_types->name }}</h4>
                                                        <span class="fs-14 d-block mb-2 text-secondary">#{{ $book->room->room_number }}</span>
                                                        <span class="fs-16 text-nowrap">{{ $book->created_at->toFormattedDateString() }}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-xl-2 col-sm-5 col-6">
                                                <div class="ms-0 ms-sm-2">
                                                    ...
                                                    <span class="guest-text font-w500">{{ $book->checkin_date }}</span>
                                                </div>
                                            </div>
                                            <div class="col-xl-2 col-sm-4 col-6">
                                                <div>
                                                    <span class="d-block">Check Out</span>
                                                    <span class="guest-text font-w500">{{ $book->checkout_date }}</span>
                                                </div>
                                            </div>
                                            <div class="col-xl-2 col-sm-4 col-6">
                                                <div class="mt-sm-0 mt-3">
                                                    <span class="d-block mb-2 text-black">Price</span>	
                                                    <span class="font-w500 fs-24 text-black">K{{ $book->room->room_types->price }}<small class="fs-14 ms-2 text-secondary">/{{ $book->room->room_types->per }}</small></span>
                                                </div>
                                            </div>
                                            {{-- <div class="col-xl-2 col-sm-4 col-6">
